...
def solve_tsp(distance_dict, points, time=3):
    ...
    if model.SolCount > 0:
        ...
        route = [i for i in solution if solution[i] > 0.5]
        objective_value = model.ObjVal
        return route, objective_value
    else:
        # No feasible solution found
        if model.status == GRB.TIME_LIMIT:
            raise ("No feasible solution found within the time limit.")
        else:
            raise ("Optimization was unsuccessful. Status code:", model.status)
        return None
...
routeOPT = solve_tsp(distance_dict, points, 5)
routeNN = nearest_neighbour_v2(distance_dict, points)
print(routeOPT)
print(routeNN)
# plot_tour(points_dict, routeOPT, False)
# plot_tour(points_dict, routeNN, True)
print(calculate_distance(distance_dict, routeOPT))
print(calculate_distance(distance_dict, routeNN, True))
